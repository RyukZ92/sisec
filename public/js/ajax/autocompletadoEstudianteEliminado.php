...
$tabla = 'estudiante e JOIN datos_personales dp ON dp.dni = e.dni';
$datos = array("dp.dni", "nombre");
...
for($i=0; $i<count($_dni);$i++){
    ...
    $estudiante = $nombre[0] . " " . $apellido[0];
    $dni = str_replace($_POST['keyword'], '<b>'.$_POST['keyword'].'</b>', $_dni[$i]['dni']);
    echo '<a href="javascript:void()" style="text-decoration:none; color:#000;"><li onclick="set_item(\''.str_replace("'", "\'", $_dni[$i]['dni']).'\')">' . $dni . ' ' . $estudiante . '</li></a>';
}
?>