...
/**
 * =============================================================================
 * @Descripcion: Controlador para configurar los datos de la inyección sql
 * sesión iniciada
...
if($_POST["guardar"]) {
    if(Helper::validarCamposVacios(array($_POST["url"], $_POST["nombre_campo"], 
        ...
        $contenido .= "## Archivo de Configuración de Inyección SQL" . PHP_EOL;
        ...
        $contenido .= "archivo = " . RUTARAIZ . "files/injections-sql/inyeccion-sql-personalizada.txt" . PHP_EOL;
       ...
       $archivo = $objArchivo->escrbir("config/config.tests/config.inyeccion-sql.ini", $contenido);
        if($archivo) {
          $notificacion = $alerta["actualizacion_exitosa"];
          unset($_POST);
           header("refresh:6;". URLBASE . "configurar/inyeccion-sql");
        } else {
...
require "application/views/configuracion/evaluacion/configurarInyeccionSqlView.phtml";